    ...
    while retries < max_retries:
        try:
            ...
            if result:
                ...
                print(operation_name + " failed, retrying... (" + str(retries + 1) + "/" + str(max_retries) + ")")
                retries += 1
                time.sleep(retry_delay * retries)
        except Exception as e:
            print(operation_name + " raised exception: " + str(e) + ", retrying... (" + str(retries + 1) + "/" + str(max_retries) + ")")
            retries += 1
            time.sleep(retry_delay * retries)
    
    print("ERROR: " + operation_name + " failed after " + str(max_retries) + " retries")
    return False
...
def gclient_sync():
    ...
    def sync_operation():
        if ("windows" == base.host_platform()):
            ...
            result = base.cmd("gclient", ["sync", "--force"], True)
        else:
            result = base.cmd("gclient", ["sync"], True)
        return result['returncode'] == 0
    
    return retry_operation(sync_operation, "gclient sync")
...
def make():
  ...
  if not base.is_dir("v8/out.gn"):
    ...
    if ("linux" == base.host_platform()):
      ...
      def linux_sync_operation():
          result = base.cmd("gclient", ["sync", "--no-history"])
          return result['returncode'] == 0
      
      if not retry_operation(linux_sync_operation, "gclient sync --no-history"):
          os.chdir(old_cur)
          os.environ.clear()
          os.environ.update(old_env)
          return
...
def make_xp():
  ...
  # fetch
  if not base.is_dir("v8"):
    def fetch_v8_xp_operation():
        result = base.cmd("./depot_tools/fetch", ["v8"], True)
        if result['returncode'] != 0:
            return False
        result = base.cmd("./depot_tools/gclient", ["sync", "-r", "4.10.253"], True)
        if result['returncode'] != 0:
            return False
        base.delete_dir_with_access_error("v8/buildtools/win")
        base.cmd("git", ["config", "--system", "core.longpaths", "true"])
    ...
    if not retry_operation(fetch_v8_xp_operation, "Fetch v8 for XP"):
      os.chdir(old_cur)
      os.environ.clear()
      os.environ.update(old_env)
      return
