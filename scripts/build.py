...
```
########################################################
# Модуль build.py - сборка Qt-проектов через qmake     #
# Python 3.5. Генерация Makefile и сборка проектов     #
# для различных платформ. Ключевые функции:            #
# • make_pro_file() - сборка .pro файлов с поддержкой  #
#   Android/iOS окружения                              #
# • make() - сборка build.pro с обработкой Document    #
#   Builder на Windows                                 #
#                                                      #
# Особенности:                                         #
# • Автонастройка toolchain для Android/NDK            #
# • Патчинг Xcode для iOS сборки                       #
# • Параллельная сборка (multiprocess)                 #
# • Очистка перед сборкой (clean/distclean)            #
# • Генерация уникальных Makefile по платформе         #
#                                                      #
# Импортируется:                                       #
# • automate.py - основная сборка проектов             #
# • Модули в /modules/desktop/, /modules/server/       #
# • Скрипты сборки конкретных продуктов                #
########################################################
```
